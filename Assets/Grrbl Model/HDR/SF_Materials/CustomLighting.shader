    ...
    SubShader {
        Pass {
            ...
            fixed4 frag(vertexOutput i) : COLOR {
                ...
                fixed specularCutoff = _LightColor0.xyz * _SpecColor.rgb * max(0.0, dot(normalDirection, lightDirection)) * pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess);
                ...
                //Rim lighting
                float rim = 1 - saturate(dot(normalize(i.viewDir), normalDirection));
                float3 rimLighting =  _RimColor * saturate(dot(normalDirection, normalDirection)) * pow(rim, _RimPower);
